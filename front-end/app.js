...
const urlListarPostagens = 'http://localhost:3000/postagens';
...
async function incluirPostagem() {
    ...
    const inputTitulo = getById('titulo');
    const inputConteudo = getById('conteudo');
...
async function editarPostagem(id, tituloAtual, textoAtual) {
...
async function listarPostagens() {
    ...
    if (!postagensElement)
        return;
    function renderPostagens(listaPostagens) {
    ...
    const selectFilterCategoria = getById('filterCategoria');
...
function mostrarAreaComentario(id, container) {
    if (container.querySelector('.area-comentario'))
        return;
...
async function enviarComentario(id, container) {
